    ...
    <>
      <section className="detail-section">
          ...
          <ul className="session-list">
            {sessions.map((s) => (
              <li key={s.deviceId} className="definition-row surface-card">
                <div>
                  <strong>{s.userAgent?.slice(0, 60) || "Device"}</strong>
                  <div className="muted">Last active {new Date(s.lastActiveAt).toLocaleString()}</div>
                </div>
              </li>
            ))}
          </ul>
        ...
        <button
          ...
          onClick={() => {
            void signOutEverywhere({ currentDeviceId: getDeviceId() }).then(
      ...
      <ConfirmDialog
        ...
        onConfirm={() => doDelete()}        title="Delete account"
